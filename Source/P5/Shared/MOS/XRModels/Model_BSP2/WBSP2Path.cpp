...
int CXR_Model_BSP2::IntPath_GetPortalLeaf(const CVec3Dfp32& _v)
{
...
static fp32 BoxDistance(const CVec3Dfp32& v, const CBox3Dfp32 _Box)
{
	CVec3Dfp32 dV;
	for(int i = 0; i < 3; i++)
	{
...
static void GetMidPoint(const CVec3Dfp32* _pV, const uint32* _piV, int _nV, CVec3Dfp32& _Dst)
{
	CVec3Dfp32 V(0);
	for(int v = 0; v < _nV; v++)
		V += _pV[_piV[v]];
...
static fp32 GetFenceDistance(const CVec3Dfp32& _v0, const CVec3Dfp32& _v1, const CVec3Dfp32* _pV, const uint32* _piV, int _nV)
{
	CPlane3Dfp32 Plane;
...
int CXR_Model_BSP2::IntPath_FindPortal(int _iPL, const CVec3Dfp32& _v0, const CVec3Dfp32& _v1)
{
	CBSP2_PortalLeaf* pPL = &m_lPortalLeaves[_iPL];
	CVec3Dfp32 dV;
	_v1.Sub(_v0, dV);
...
int CXR_Model_BSP2::Path_GetNeighbourPortal(int _iPL1, int _iPL2)
{
	CBSP2_PortalLeaf* pPL = &m_lPortalLeaves[_iPL1];
	for(int p = 0; p < pPL->m_nPortals; p++)
	{
		int iP = m_liPortals[p + pPL->m_iiPortals];
		CBSP2_Portal* pP = &m_lPortals[iP];
...
static bool Path_bComplete = false;
static uint16* Path_piPL = NULL;
static int Path_niPL = 0;
static int Path_MaxPL = 0;
static int Path_iPLDestination = 0;
static int Path_MaxEnterFlg = 0;
static int Path_MaxExitFlg = 0;

static int Path_NodeCounter = 0;
static int Path_PortalCounter = 0;

void CXR_Model_BSP2::IntPath_Build_r(int _iPL, int _iPathPL, const CVec3Dfp32& _v0, const CVec3Dfp32& _v1)
{
...
//ConOutL(CStrF("(CXR_Model_BSP2::IntPath_Build_r) Path[%d] = %d", _iPathPL, _iPL));
...
/*	int ip = Path_GetNeighbourPortal(_iPL, Path_iPLDestination);
	...
	for(i = 0; i < pPL->m_nPortals; i++)
	{
		...
		CBSP2_PortalLeafExt* pDstPL = &m_lPortalLeaves[iDstPL];
...
//		pP->m_Flags |= XW_PORTAL_SEARCHED;
		...
		{
//			ConOutL(CStrF("(CXR_Model_BSP2::IntPath_Build_r) Recurse %d => %d", _iPL, m_lNodes[pP->m_iNodeBack].m_iNavigationLeaf));
			IntPath_Build_r(m_lNodes[pP->m_iNodeBack].m_iNavigationLeaf, _iPathPL+1, Mid, _v1);
		}
		else
		{
//			ConOutL(CStrF("(CXR_Model_BSP2::IntPath_Build_r) Recurse %d => %d", _iPL, m_lNodes[pP->m_iNodeFront].m_iNavigationLeaf));
...
int CXR_Model_BSP2::IntPath_GetLowestPathIndexPortalLeaf(int _iPL)
{
	...
	for(int p = 0; p < pPL->m_nPortals; p++)
	{
		...
		CBSP2_PortalLeafExt* pPLOther = &m_lPortalLeaves[iPLOther];
...
int CXR_Model_BSP2::IntPath_GetHighestPathIndexPortalLeaf(int _iPL)
{
	CBSP2_PortalLeafExt* pPL = &m_lPortalLeaves[_iPL];
...
int CXR_Model_BSP2::IntPath_Optimize(uint16* _piPL, int _niPL)
{
	int j = 0;
...
int CXR_Model_BSP2::Path_Create(const CVec3Dfp32& _v0, const CVec3Dfp32& _v1, int _Flags, uint16* _piPL, int _MaxPL)
{
	ConOutL("(CXR_Model_BSP2::Path_Create) Src " + _v0.GetString() + ", Dst " + _v1.GetString());
...
/*	{
	...
	if (!iPL0)
	{
		ConOutL("(CXR_Model_BSP2::Path_Create) No source PL.");
		return false;
	}
	...
	if (!iPL1)
	{
		ConOutL("(CXR_Model_BSP2::Path_Create) No destination PL.");
		return false;
	}
	...
	if (iPL0 == iPL1)
	{
		ConOutL("(CXR_Model_BSP2::Path_Create) No source == destination PL.");
		return false;
	}

ConOutL(CStrF("(CXR_Model_BSP2::Path_Create) iPL1 %d, iPL2 %d", iPL0, iPL1));
	...
	{
		CBSP2_PortalLeafExt* pPL = m_lPortalLeaves.GetBasePtr();
		for(int p = 0; p < Path_niPL; p++)
			pPL[Path_piPL[p]].m_Flags |= XW_PORTAL_PATH;
	}

ConOutL(CStrF("(CXR_Model_BSP2::Path_Create) bComplete %d, Len %d, RecurseCount %d, PortalCount %d", Path_bComplete, Path_niPL, Path_NodeCounter, Path_PortalCounter));
	int nSearch1 = IntPath_Optimize(Path_piPL, Path_niPL);
	bool bComplete1 = Path_bComplete;

ConOutL(CStrF("(CXR_Model_BSP2::Path_Create) Optimized len %d", Path_niPL));
...
void CXR_Model_BSP2::Path_GetPortalLeaf(int _iPortalLeaf, CXR_PathPortalLeaf& _Leaf)
{
	CBSP2_PortalLeaf* pPL = &m_lPortalLeaves[_iPortalLeaf];
...
CXR_IndexedPortal* CXR_Model_BSP2::Path_GetPortal(int _iPortal)
{

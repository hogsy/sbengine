...
* Vital statistics:
  ...
  Maximum overhead wastage per allocated chunk: normally 15 bytes
	 ...
	 2. For requests >= GGV->mmap_threshold that are serviced via
...
/*
    ...
    noticeably. Calling malloc_stats or mallinfo with DLM_DEBUG set will
    attempt to check every non-mmapped allocated and free chunk in the
    course of computing the summmaries. (By nature, mmapped regions
    cannot be checked very much automatically.)

    Setting DLM_DEBUG may also be helpful if you are trying to modify
...
#else
Void_t* __cdecl memset();
Void_t* __cdecl memcpy();
#endif
#endif
...
struct DLM_GlobalVars_Gm
{
	GmListElement* head;
	unsigned int gNextAddress;
...
extern struct DLM_GlobalVars_Gm *GetGlobalVars_GM();
...
GmListElement* makeGmListElement (void* bas)
{
	...
	{
		This->base = bas;
		This->next = GetGlobalVars_GM()->head;
		GetGlobalVars_GM()->head = This;
	}
	return This;
}
...
{
	BOOL rval;
	M_ASSERT ((GetGlobalVars_GM()->head == NULL) || (GetGlobalVars_GM()->head->base == (void*)GetGlobalVars_GM()->gAddressBase), "");
	if (GetGlobalVars_GM()->gAddressBase && (GetGlobalVars_GM()->gNextAddress - GetGlobalVars_GM()->gAddressBase))
	{
		rval = VirtualFree ((void*)GetGlobalVars_GM()->gAddressBase,
							GetGlobalVars_GM()->gNextAddress - GetGlobalVars_GM()->gAddressBase,
							MEM_DECOMMIT);
	M_ASSERT (rval, "");
	}
	while (GetGlobalVars_GM()->head)
	{
		GmListElement* next = GetGlobalVars_GM()->head->next;
		rval = VirtualFree (GetGlobalVars_GM()->head->base, 0, MEM_RELEASE);
		M_ASSERT (rval, "");
		LocalFree (GetGlobalVars_GM()->head);
		GetGlobalVars_GM()->head = next;
	}
}
...
void* wsbrk (long size)
{
	...
	if (size > 0)
	{
		if (GetGlobalVars_GM()->gAddressBase == 0)
		{
			GetGlobalVars_GM()->gAllocatedSize = max (RESERVED_SIZE, AlignPage (size));
			GetGlobalVars_GM()->gNextAddress = GetGlobalVars_GM()->gAddressBase =
				(unsigned int)VirtualAlloc (NULL, GetGlobalVars_GM()->gAllocatedSize,
											MEM_RESERVE, PAGE_NOACCESS);
		} else if (AlignPage (GetGlobalVars_GM()->gNextAddress + size) > (GetGlobalVars_GM()->gAddressBase +
GetGlobalVars_GM()->gAllocatedSize))
		{
			long new_size = max (NEXT_SIZE, AlignPage (size));
			void* new_address = (void*)(GetGlobalVars_GM()->gAddressBase+GetGlobalVars_GM()->gAllocatedSize);
			do
			{
				...
				GetGlobalVars_GM()->gAddressBase = GetGlobalVars_GM()->gNextAddress =
			...
			while (GetGlobalVars_GM()->gAddressBase == 0);

			M_ASSERT (new_address == (void*)GetGlobalVars_GM()->gAddressBase, "");

			GetGlobalVars_GM()->gAllocatedSize = new_size;

			if (!makeGmListElement ((void*)GetGlobalVars_GM()->gAddressBase))
				return (void*)-1;
		}
		if ((size + GetGlobalVars_GM()->gNextAddress) > AlignPage (GetGlobalVars_GM()->gNextAddress))
		{
			void* res;
			res = VirtualAlloc ((void*)AlignPage (GetGlobalVars_GM()->gNextAddress),
								(size + GetGlobalVars_GM()->gNextAddress -
								 AlignPage (GetGlobalVars_GM()->gNextAddress)),
								MEM_COMMIT, PAGE_READWRITE);
			if (res == 0)
				return (void*)-1;
		}
		tmp = (void*)GetGlobalVars_GM()->gNextAddress;
		GetGlobalVars_GM()->gNextAddress = (unsigned int)tmp + size;
		return tmp;
	}
	else if (size < 0)
	{
		unsigned int alignedGoal = AlignPage (GetGlobalVars_GM()->gNextAddress + size);
		/* Trim by releasing the virtual memory */
		if (alignedGoal >= GetGlobalVars_GM()->gAddressBase)
		{
			VirtualFree ((void*)alignedGoal, GetGlobalVars_GM()->gNextAddress - alignedGoal,
						 MEM_DECOMMIT);
			GetGlobalVars_GM()->gNextAddress = GetGlobalVars_GM()->gNextAddress + size;
			return (void*)GetGlobalVars_GM()->gNextAddress;
		}
		else
		{
			VirtualFree ((void*)GetGlobalVars_GM()->gAddressBase, GetGlobalVars_GM()->gNextAddress - GetGlobalVars_GM()->gAddressBase,
						 MEM_DECOMMIT);
			GetGlobalVars_GM()->gNextAddress = GetGlobalVars_GM()->gAddressBase;
			return (void*)-1;
		}
...
#define bin_at(i)      ((mbinptr)((char*)&(GetGlobalVars()->av_[2*(i) + 2]) - 2*SIZE_SZ))
...
#define sbrked_mem  (GetGlobalVars()->current_mallinfo.arena)
...
struct DLM_GlobalVars
{
	 ...
	 */
	const char *debug_file_min;
	const char *debug_file_max;
	int recurs;
	mchunkptr lowest_chunk;
	mbinptr av_[NAV * 2 + 2];
	...
	void Init()
	{
		...
		{
			av_[i*2] = av_[i*2+1] = ((mbinptr)((char*)&(av_[2*(i-1) + 2]) - 2*SIZE_SZ));
		}
	}
};

extern DLM_GlobalVars *GetGlobalVars();
#define GGV GetGlobalVars()
...
#endif
{
  ...
  if(GGV->n_mmaps >= GGV->n_mmaps_max) return 0; /* too many regions */
  ...
  GGV->n_mmaps++;
  if (GGV->n_mmaps > max_n_mmaps) max_n_mmaps = GGV->n_mmaps;
  ...
  if ((unsigned long)(mmapped_mem + sbrked_mem) > (unsigned long)GGV->max_total_mem)
    GGV->max_total_mem = mmapped_mem + sbrked_mem;
  return p;
}
...
#endif
{
  ...
  /* Pad request with GGV->top_pad plus minimal overhead */

  INTERNAL_SIZE_T    sbrk_size     = nb + GGV->top_pad + MINSIZE;
  ...
  else
  {
    ...
    if (GGV->sbrk_base == (char*)(-1))  /* First time through. Record base */
      GGV->sbrk_base = lim;
  ...
  if ((unsigned long)sbrked_mem > (unsigned long)GGV->max_sbrked_mem)
    GGV->max_sbrked_mem = sbrked_mem;
  if ((unsigned long)(GGV->mmapped_mem + sbrked_mem) > (unsigned long)GGV->max_total_mem)
    GGV->max_total_mem = GGV->mmapped_mem + sbrked_mem;
...
#endif
{
  ...
  */
  ...
  if ( (remainder_size = chunksize(top) - nb) < (long)MINSIZE)
  {
    ...
    if ((unsigned long)nb >= (unsigned long)GGV->mmap_threshold &&
...
#endif
{
  ...
  if (next == top)                            /* merge with top */
  {
    ...
    if ((unsigned long)(sz) >= GGV->trim_threshold)
      malloc_trim(GGV->top_pad);
    return;
  }
...
#endif
{
  ...
  else
  {
	...
	{
	  sbrked_mem = current_lim - GGV->sbrk_base;
	  set_head(top, top_size | PREV_INUSE);
...
/* Utility to update GGV->current_mallinfo for malloc_stats and mallinfo() */
...
static void malloc_update_mallinfo(void)
{
  ...
  GGV->current_mallinfo.ordblks = navail;
  GGV->current_mallinfo.uordblks = sbrked_mem - avail;
  GGV->current_mallinfo.fordblks = avail;
  GGV->current_mallinfo.hblks = GGV->n_mmaps;
  GGV->current_mallinfo.hblkhd = GGV->mmapped_mem;
  GGV->current_mallinfo.keepcost = chunksize(top);
...
void malloc_stats(void)
{
  ...
  fprintf(stderr, "max system bytes = %10u\n",
	  (unsigned int)(GGV->max_total_mem));
  fprintf(stderr, "system bytes     = %10u\n",
	  (unsigned int)(sbrked_mem + GGV->mmapped_mem));
  fprintf(stderr, "in use bytes     = %10u\n",
	  (unsigned int)(GGV->current_mallinfo.uordblks + GGV->mmapped_mem));
#if HAVE_MMAP
...
#endif
{
  switch(param_number)
  {
    case M_TRIM_THRESHOLD:
      GGV->trim_threshold = value; return 1;
    case M_TOP_PAD:
      GGV->top_pad = value; return 1;
    case M_MMAP_THRESHOLD:
      GGV->mmap_threshold = value; return 1;
    case M_MMAP_MAX:
#if HAVE_MMAP
      GGV->n_mmaps_max = value; return 1;
#else
      if (value != 0) return 0; else  GGV->n_mmaps_max = value; return 1;
#endif
    case M_SCANHEAP:
#ifdef DLM_DEBUG2
      scanheap = value;
...
History:
	...
	Wolfram Gloger (Gloger@lrz.uni-muenchen.de).
      ...
      * Fix error occuring when initial GGV->sbrk_base not word-aligned.
      * Rely on page size for units instead of SBRK_UNIT to
	avoid surprises about sbrk alignment conventions.
      * Add mallinfo, mallopt. Thanks to Raymond Nijssen
	(raymond@es.ele.tue.nl) for the suggestion.
      * Add `pad' argument to malloc_trim and GGV->top_pad mallopt parameter.

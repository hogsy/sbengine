...
{
	return _Val - __fcfid(__fctidz(_Val));
}
...
{
	fp32 Value = __fcfid(__fctidz(_Value));
	fp32 Frac = _Value - Value;
	return Value - __fsel(Frac, 0.0f, 1.0f);
}
...
{
	fp64 Value = __fcfid(__fctidz(_Value));
	fp64 Frac = _Value - Value;
	return Value + __fsel(-Frac, 0.0f, 1.0f);
}

...
/*
	small addition to dlmalloc.c to make it work with a static heap ...
*/
#define __STD_C              1
#define USE_DL_PREFIX        1
#define LACKS_SYS_PARAM_H    1
#define LACKS_SYS_MMAN_H     1
#define MORECORE             dummy_sbrk
#define MORECORE_CONTIGUOUS  1
#define MORECORE_FAILURE     ((void*)(-1))
#if	defined(PLATFORM_DOLPHIN)
#define memcpy               std::memcpy
#define memset               std::memset
#define TRIM_FASTBINS        1 //ar-test
...
// call this before calling dlmalloc
void dlmalloc_initstatic(void* _pHeap, int _nSize);
int  dlmalloc_heapfree();

static void*  s_pHeap      = NULL;
static int    s_nHeapSize  = 0;
static int    s_nBytesUsed = 0;
...
int dlmalloc_heapfree()
{
	return (s_nHeapSize - s_nBytesUsed);
}

static void* dummy_sbrk(int _Size)
{
	if ((s_pHeap == NULL) || ((s_nBytesUsed + _Size) > s_nHeapSize))
		return MORECORE_FAILURE;

	void* pReturn = (char*)s_pHeap + s_nBytesUsed;
	s_nBytesUsed += _Size;
	return pReturn;
}
...
#endif
/*
	... and below follows original dlmalloc.c ...
*/
#endif
...
struct mallinfo {
  ...
  INTERNAL_SIZE_T	lrgblk;	/* largest block available */
};
...
struct mallinfo mALLINFo()
{
  ...
  int nblocks;
  int nfastblocks;
  INTERNAL_SIZE_T largest = s_nHeapSize - s_nBytesUsed;
  ...
  for (i = 0; i < NFASTBINS; ++i) {
    for (p = av->fastbins[i]; p != 0; p = p->fd) {
      INTERNAL_SIZE_T chunk = chunksize(p);
      ++nfastblocks;
      fastavail += chunk;
      if( chunk > largest )
      	largest	= chunk;

...
private:
	...
	getRand()
	{
		MAUTOSTRIP(CXR_Model_DavidTest_getRand, 0);
	...
	getAbs(fp32 x)
	{
		MAUTOSTRIP(CXR_Model_DavidTest_getAbs, 0);
		return ((x < 0.0f) ? -x : x);
	}
	...
	getSign(fp32 x)
	{
		MAUTOSTRIP(CXR_Model_DavidTest_getSign, 0);
	...
	fp32
	getSinRand(fp32* boost, fp32 timescale, fp32 time_adjustment = 0.0f)
	{
		MAUTOSTRIP(CXR_Model_DavidTest_getSinRand, 0);
	...
	{
		MAUTOSTRIP(CXR_Model_DavidTest_getParticlePos, CVec3Dfp32(0,0,0));
		CVec3Dfp32 pos;
	...
	{
		MAUTOSTRIP(CXR_Model_DavidTest_addParticle, MAUTOSTRIP_VOID);
	...
	{
		MAUTOSTRIP(CXR_Model_DavidTest_addStrip, MAUTOSTRIP_VOID);
	...
	{
		MAUTOSTRIP(CXR_Model_DavidTest_fillParticleArray, MAUTOSTRIP_VOID);
		...
		vanish_duration = 0.1f * duration;
		maxTimeOffset = 0.4f * duration;
		particle_duration = duration - trail_duration - vanish_duration - maxTimeOffset;
...
//		ConOut(CStrF("destination = <%f, %f, %f>", destination_pos.k[0], destination_pos.k[1], destination_pos.k[2]));
		...
		posbasetimescale = 0.75f / duration;
		postimescale = 1.0f / duration;
		phase_shift = 0.5f;

		octaves = 8;

		fp32 posbaseboostarrapos[8];
		posbaseboostarrapos[0] = 1.0f;
		posbaseboostarrapos[1] = 1.0f;
		posbaseboostarrapos[2] = 0.0f;
		posbaseboostarrapos[3] = 0.0f;
		posbaseboostarrapos[4] = 0.0f;
		posbaseboostarrapos[5] = 0.0f;
		posbaseboostarrapos[6] = 0.0f;
		posbaseboostarrapos[7] = 0.0f;
		posbaseboost = posbaseboostarrapos;

		fp32 posboostarrapos[8];
		posboostarrapos[0] = 1.0f;
		posboostarrapos[1] = 1.0f;
		posboostarrapos[2] = 0.0f;
		posboostarrapos[3] = 0.0f;
		posboostarrapos[4] = 0.0f;
		posboostarrapos[5] = 0.0f;
		posboostarrapos[6] = 0.0f;
		posboostarrapos[7] = 0.0f;
		posboost = posboostarrapos;
	...
	void
	fillParticle(int32 index)
	{
		MAUTOSTRIP(CXR_Model_DavidTest_fillParticle, MAUTOSTRIP_VOID);
		fp32 alpha = 1.0f;
...
//			alpha = 1.0f;
			...
			fp32 ybase = getSinRand(posbaseboost, posbasetimescale, t - striptimeoffset);
			fp32 zbase = getSinRand(posbaseboost, posbasetimescale, t - striptimeoffset);

			randseed = randseed_temp;

			fp32 y = getSinRand(posboost, postimescale, t - striptimeoffset);
			fp32 z = getSinRand(posboost, postimescale, t - striptimeoffset);
...
//			strip.m_Color = 0x80FFFFFF;
	...
	{
		MAUTOSTRIP(CXR_Model_DavidTest_OnCreate, MAUTOSTRIP_VOID);
...
//			m_iSurface = GetSurfaceID("Effects_Flare01");
	...
	{
		MAUTOSTRIP(CXR_Model_DavidTest_Render, MAUTOSTRIP_VOID);
